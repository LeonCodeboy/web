...
$stat_desc = array("В ожидании","Начат","В процессе","На удержании","Просрочен","Завершен");
...
<div class="col-lg-12">
				...
				<div class="col-md-12">
					<div class="row">
						...
						<div class="col-md-6">
							...
							<dl>
								<dt><b class="border-bottom border-primary">Дата окончания</b></dt>
								...
								<dd>
									<?php
									  if($stat[$status] =='Pending'){
									  	echo "<span class='badge badge-secondary'>{$stat_desc[$status]}</span>";
									  }elseif($stat[$status] =='Started'){
									  	echo "<span class='badge badge-primary'>{$stat_desc[$status]}</span>";
									  }elseif($stat[$status] =='On-Progress'){
									  	echo "<span class='badge badge-info'>{$stat_desc[$status]}</span>";
									  }elseif($stat[$status] =='On-Hold'){
									  	echo "<span class='badge badge-warning'>{$stat_desc[$status]}</span>";
									  }elseif($stat[$status] =='Over Due'){
									  	echo "<span class='badge badge-danger'>{$stat_desc[$status]}</span>";
									  }elseif($stat[$status] =='Done'){
									  	echo "<span class='badge badge-success'>{$stat_desc[$status]}</span>";
									  }
									?>
								</dd>
							</dl>
	...
	<div class="row">
		<div class="col-md-12">
			<div class="card">
				<div class="card-header">
					<b>Активность участников</b>
					<div class="card-tools">
						<button class="btn btn-primary bg-gradient-primary btn-sm" type="button" id="new_productivity"><i class="fa fa-plus"></i> Новый этап</button>
					</div>
				</div>
				<div class="card-body">
					<?php 
					...
					?>
						<div class="post">
		                      ...
		                      <div class="user-block">
								...
								<?php endif; ?>
		                        <img class="img-circle img-bordered-sm" src="assets/uploads/<?php echo $row['avatar'] ?>" alt="Изображение пользователя">
		                        <span class="username">
		                        ...
		                        <span class="description">
		                        	...
		                        	<span class="fa fa-user-clock"></span>
                      				<span>Дата начала: <b><?php echo date('h:i A',strtotime($row['date'].' '.$row['start_time'])) ?></b></span>
		                        	<span> | </span>
                      				<span>Дата завершения: <b><?php echo date('h:i A',strtotime($row['date'].' '.$row['end_time'])) ?></b></span>
	                        	</span>
...
<script>
	...
	_conf("Вы действительно хотите удалить задачу?","delete_task",[$(this).attr('data-id')])
	})
	$('#new_productivity').click(function(){
		uni_modal("<i class='fa fa-plus'></i> Новый этап","manage_progress.php?pid=<?php echo $id ?>",'large')
	})
	$('.manage_progress').click(function(){
		uni_modal("<i class='fa fa-edit'></i> Редактировать этап","manage_progress.php?pid=<?php echo $id ?>&id="+$(this).attr('data-id'),'large')
	})
	$('.delete_progress').click(function(){
	_conf("Вы действительно хотите удалить этап?","delete_progress",[$(this).attr('data-id')])
	})
